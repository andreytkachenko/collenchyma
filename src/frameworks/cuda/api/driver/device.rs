...
impl API {
    ...
    pub fn load_device_list() -> Result<Vec<Device>, Error> {
        ...
        r#try!(unsafe {API::ffi_device_get_count(&mut device_counter)});
    ...
    pub fn load_device_info(device: &Device, info: CUdevice_attribute) -> Result<DeviceInfo, Error> {
        match info {
            CUdevice_attribute::CU_DEVICE_NAME => {
                ...
                r#try!(unsafe { API::ffi_device_get_name(name.as_mut_ptr(), 1024, device.id_c()) });
            ...
            _ => {
                ...
                r#try!(unsafe { API::ffi_device_get_attribute(&mut value, info, device.id_c()) });
                let mut buf = vec![];
