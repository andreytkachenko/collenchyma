...
use std::ptr;
use libc;
use crate::frameworks::opencl::{API, Error, Event, Context, Memory, MemoryFlags, Queue};
...
impl API {
    ...
    pub fn release_memory(memory: &mut Memory) -> Result<(), Error> {
        Ok(r#try!(unsafe {API::ffi_release_mem_object(memory.id_c())}))
    }
